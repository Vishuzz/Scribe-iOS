...
final class TableViewTemplateViewController: BaseTableViewController {
    ...
    private var langCode: String {
        guard let parentSection
        else {
            return ""
        }

        guard case let .specificLang(lang) = parentSection.sectionState
        else {
            return "de"
        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        for visibleCell in tableView.visibleCells {
            ...
            guard let wrapperCell = visibleCell as? WrapperCell,
                  let innerCell = wrapperCell.wrappedCell as? InfoChildTableViewCell
            else {
                continue
            }

            // Now check if it's a translate lang section.
            guard innerCell.section?.sectionState == .translateLang
            else {
                continue
            }

            let langTranslateLanguage = getKeyInDict(
                givenValue: userDefaults.string(forKey: langCode + "TranslateLanguage") ?? "en",
                dict: languagesAbbrDict
            )
            let currentLang = "i18n.app._global." + langTranslateLanguage.lowercased()
            innerCell.subLabel.text = NSLocalizedString(
...
extension TableViewTemplateViewController {
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath)
        ...
        cell.configure(
            withCellNamed: "InfoChildTableViewCell", section: setting,
